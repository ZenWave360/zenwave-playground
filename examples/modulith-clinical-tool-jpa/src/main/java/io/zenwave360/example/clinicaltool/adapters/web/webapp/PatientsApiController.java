...
public class PatientsApiController implements PatientsApi {
    ...
    public ResponseEntity<PatientDTO> loadPatient(String hisNumber, String phoneNumber) {
        log.debug("REST request to loadPatient: {}, {}", hisNumber, phoneNumber);var patient =  patientsService.loadPatient(phoneNumber, hisNumber);
    ...
    public ResponseEntity<PatientDTO> partialPatientUpdate(String hisNumber, String phoneNumber, Map input) {
        log.debug("REST request to partialPatientUpdate: {}, {}, {}", hisNumber, phoneNumber, input);var patient =  patientsService.partialPatientUpdate(phoneNumber, hisNumber, input);
        PatientDTO responseDTO = mapper.asPatientDTO(patient);
        return ResponseEntity.status(200).body(responseDTO);
    }
    ...
    public ResponseEntity<PatientDTO> createPatient(PatientDTO reqBody) {
        log.debug("REST request to createPatient: {}", reqBody);var input = mapper.asPatient(reqBody);var patient =  patientsService.createPatient(input);
        PatientDTO responseDTO = mapper.asPatientDTO(patient);
    ...
    public ResponseEntity<PatientDTO> updatePatient(Long id, PatientDTO reqBody) {
        log.debug("REST request to updatePatient: {}, {}", id, reqBody);var input = mapper.asPatient(reqBody);var patient =  patientsService.updatePatient(id, input);
    ...
    public ResponseEntity<PatientDTO> getPatient(Long id) {
        log.debug("REST request to getPatient: {}", id);var patient =  patientsService.getPatient(id);
        if (patient.isPresent()) {
            PatientDTO responseDTO = mapper.asPatientDTO(patient.get());
            return ResponseEntity.status(200).body(responseDTO);
        } else {
