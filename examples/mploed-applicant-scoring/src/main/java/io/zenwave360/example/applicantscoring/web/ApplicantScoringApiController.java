...
public class ApplicantScoringApiController implements ApplicantScoringApi {
    ...
    public ResponseEntity<ApplicantScoringDTO> getApplicationScoring(Long id) {
        log.debug("REST request to getApplicationScoring: {}", id);var applicantScoring =  applicantScoringService.getApplicationScoring(id);
    ...
    public ResponseEntity<ApplicantScoringDTO> updateApplicantScoring(Long id, ApplicantScoringInputDTO reqBody) {
        log.debug("REST request to updateApplicantScoring: {}, {}", id, reqBody);var input = mapper.asApplicantScoringInput(reqBody);var applicantScoring =  applicantScoringService.updateApplicantScoring(id, input);
        if (applicantScoring.isPresent()) {
            ApplicantScoringDTO responseDTO = mapper.asApplicantScoringDTO(applicantScoring.get());
            return ResponseEntity.status(200).body(responseDTO);
        } else {
    ...
    public ResponseEntity<ApplicantScoringDTO> createApplicantScoring(ApplicationNumberInputDTO reqBody) {
        log.debug("REST request to createApplicantScoring: {}", reqBody);var input = mapper.asApplicationNumberInput(reqBody);var applicantScoring =  applicantScoringService.createApplicantScoring(input);
    ...
    public ResponseEntity<ApplicantScoringDTO> updateCity(Long id, CityInputDTO reqBody) {
        log.debug("REST request to updateCity: {}, {}", id, reqBody);var input = mapper.asCityInput(reqBody);var applicantScoring =  applicantScoringService.updateCity(id, input);
        if (applicantScoring.isPresent()) {
            ApplicantScoringDTO responseDTO = mapper.asApplicantScoringDTO(applicantScoring.get());
            return ResponseEntity.status(201).body(responseDTO);
        } else {
    ...
    public ResponseEntity<ApplicantScoringDTO> updateBalanceAtBank(Long id, BalanceAtBankInputDTO reqBody) {
        log.debug("REST request to updateBalanceAtBank: {}, {}", id, reqBody);var input = mapper.asBalanceAtBankInput(reqBody);var applicantScoring =  applicantScoringService.updateBalanceAtBank(id, input);
        if (applicantScoring.isPresent()) {
            ApplicantScoringDTO responseDTO = mapper.asApplicantScoringDTO(applicantScoring.get());
            return ResponseEntity.status(201).body(responseDTO);
        } else {
