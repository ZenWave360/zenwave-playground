...
public class ServicesInMemoryConfig extends RepositoriesInMemoryConfig {
    ...
    protected final HospitalServiceImpl hospitalService = new HospitalServiceImpl(hospitalRepository(), doctorRepository(), eventPublisher
        );
    protected final PatientsServiceImpl patientsService = new PatientsServiceImpl(patientRepository(), provisionalPatientRepository(), eventPublisher
        );
    @Bean
	...
	public void reloadTestData() {
		...
		var provisionalPatients = _provisionalPatients != null? _provisionalPatients : testDataLoader.loadCollectionTestDataAsObjects(ProvisionalPatient.class);
