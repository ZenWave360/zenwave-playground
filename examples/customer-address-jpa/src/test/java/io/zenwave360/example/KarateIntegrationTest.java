...
class KarateIntegrationTest {
    ...
    void run() throws Exception {
        ...
        String karateEnv =
                defaultString(System.getProperty("karate.env"), "local").toLowerCase();
    ...
    public static void moveJUnitReports(String karateReportDir, String surefireReportDir) throws IOException {
        ...
        Collection<File> xmlFiles = Files.find(
                        Paths.get(karateReportDir),
                        Integer.MAX_VALUE,
                        (filePath, fileAttr) ->
    ...
    private RuntimeHook coverageRuntimeHook = new RuntimeHook() {
        ...
        public void afterHttpCall(HttpRequest request, Response response, ScenarioRuntime sr) {
            scenarioHttpCalls.add(
                    String.format("%s %s %s", request.getMethod(), request.getUrl(), response.getStatus()));
        }
