...
import io.zenwave360.example.clinicaltool.adapters.web.webapp.mappers.*;
...
public class HospitalApiController implements HospitalApi {
    ...
    public ResponseEntity<HospitalDTO> getHospital(Long id) {
        log.debug("REST request to getHospital: {}", id);var hospital =  hospitalService.getHospital(id);
    ...
    public ResponseEntity<HospitalDTO> updateHospital(Long id, HospitalDTO reqBody) {
        log.debug("REST request to updateHospital: {}, {}", id, reqBody);var input = mapper.asHospital(reqBody);var hospital =  hospitalService.updateHospital(id, input);
        if (hospital.isPresent()) {
            HospitalDTO responseDTO = mapper.asHospitalDTO(hospital.get());
            return ResponseEntity.status(200).body(responseDTO);
        } else {
    ...
    public ResponseEntity<HospitalDTO> createHospital(HospitalDTO reqBody) {
        log.debug("REST request to createHospital: {}", reqBody);var input = mapper.asHospital(reqBody);var hospital =  hospitalService.createHospital(input);
    ...
    public ResponseEntity<List<HospitalDTO>> listHospitals() {
        log.debug("REST request to listHospitals: ");var hospital =  hospitalService.listHospitals();
    ...
    public ResponseEntity<DoctorDTO> createDoctor(DoctorDTO reqBody) {
        log.debug("REST request to createDoctor: {}", reqBody);var input = mapper.asDoctor(reqBody);var doctor =  hospitalService.createDoctor(input);
    ...
    public ResponseEntity<List<DoctorDTO>> listDoctors() {
        log.debug("REST request to listDoctors: ");var doctor =  hospitalService.listDoctors();
    ...
    public ResponseEntity<DoctorDTO> updateDoctor(Long id, DoctorDTO reqBody) {
        log.debug("REST request to updateDoctor: {}, {}", id, reqBody);var input = mapper.asDoctor(reqBody);var doctor =  hospitalService.updateDoctor(id, input);
        if (doctor.isPresent()) {
    ...
    public ResponseEntity<DoctorDTO> getDoctor(Long id) {
        log.debug("REST request to getDoctor: {}", id);var doctor =  hospitalService.getDoctor(id);
        if (doctor.isPresent()) {
            DoctorDTO responseDTO = mapper.asDoctorDTO(doctor.get());
            return ResponseEntity.status(200).body(responseDTO);
        } else {
    ...
    public ResponseEntity<List<DoctorDTO>> listHospitalDoctors(Long hospitalId) {
        log.debug("REST request to listHospitalDoctors: {}", hospitalId);var doctor =  hospitalService.listHospitalDoctors(hospitalId);
        var responseDTO = mapper.asDoctorDTOList(doctor);
        return ResponseEntity.status(200).body(responseDTO);
    }
    ...
    public ResponseEntity<List<PatientHospitalDTO>> listHospitalPatients(Long hospitalId) {
        log.debug("REST request to listHospitalPatients: {}", hospitalId);var patientHospital =  hospitalService.listHospitalPatients(hospitalId);
