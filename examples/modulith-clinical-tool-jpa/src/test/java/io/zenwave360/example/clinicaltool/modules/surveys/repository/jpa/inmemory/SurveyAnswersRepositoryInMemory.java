...
public class SurveyAnswersRepositoryInMemory extends InMemoryJpaRepository<SurveyAnswers>
        implements SurveyAnswersRepository {
    @Override
    public java.util.Optional<SurveyAnswers> findBySurveyIdAndPatientIdAndDate(
            Long surveyId, Long patientId, LocalDate date) {
        return getEntities().values().stream()
                .filter(e -> isSameValue(surveyId, readField(e, "surveyId"))
