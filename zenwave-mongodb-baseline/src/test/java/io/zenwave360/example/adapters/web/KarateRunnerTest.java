...
class KarateRunnerTest {
    ...
    static void moveJUnitReports(String karateReportDir, String surefireReportDir) throws IOException {
        new File(surefireReportDir).mkdirs();
        Collection<File> xmlFiles = Files
            .find(Paths.get(karateReportDir), Integer.MAX_VALUE,
        ...
        xmlFiles.forEach((x) -> {
            try {
                Files.copy(x.toPath(), Paths.get(surefireReportDir, "/TEST-" + x.getName()),
    ...
    private RuntimeHook coverageRuntimeHook = new RuntimeHook() {
        ...
        public void afterHttpCall(HttpRequest request, Response response, ScenarioRuntime sr) {
            scenarioHttpCalls
                .add(String.format("%s %s %s", request.getMethod(), request.getUrl(), response.getStatus()));
        }
