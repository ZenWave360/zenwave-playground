...
public class MasterDataApiController implements MasterDataApi {
    ...
    public ResponseEntity<MasterDataDTO> createMasterData(MasterDataDTO reqBody) {
        log.debug("REST request to createMasterData: {}", reqBody);var input = mapper.asMasterData(reqBody);var masterData =  masterDataService.createMasterData(input);
    ...
    public ResponseEntity<MasterDataPaginatedDTO> listMasterData(Integer page, Integer limit, List<String> sort) {
        log.debug("REST request to listMasterData: {}, {}, {}", page, limit, sort);var masterDataPage =  masterDataService.listMasterData(pageOf(page, limit, sort));
    ...
    public ResponseEntity<MasterDataDTO> getMasterData(Long id) {
        log.debug("REST request to getMasterData: {}", id);var masterData =  masterDataService.getMasterData(id);
    ...
    public ResponseEntity<MasterDataDTO> updateMasterData(Long id, MasterDataDTO reqBody) {
        log.debug("REST request to updateMasterData: {}, {}", id, reqBody);var input = mapper.asMasterData(reqBody);var masterData =  masterDataService.updateMasterData(id, input);
        if (masterData.isPresent()) {
            MasterDataDTO responseDTO = mapper.asMasterDataDTO(masterData.get());
            return ResponseEntity.status(200).body(responseDTO);
        } else {
    ...
    public ResponseEntity<List<MasterDataKeyValueDTO>> listMasterDataOfType(String type, String lang) {
        log.debug("REST request to listMasterDataOfType: {}, {}", type, lang);var masterDataKeyValue =  masterDataService.listMasterDataOfType(MasterDataType.valueOf(type), lang);
