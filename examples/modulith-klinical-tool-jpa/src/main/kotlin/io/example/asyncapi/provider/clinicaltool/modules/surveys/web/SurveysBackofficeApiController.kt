package io.example.asyncapi.provider.clinicaltool.modules.surveys.web
...
open class SurveysBackofficeApiController(
    ...
    override fun getSurvey(id: Long): ResponseEntity<SurveyDTO> {
        ...
        return if (survey.isPresent) {
            val responseDTO = mapper.asSurveyDTO(survey.get())
    ...
    override fun getQuestion(id: Long): ResponseEntity<QuestionDTO> {
        ...
        return if (question.isPresent) {
            val responseDTO = mapper.asQuestionDTO(question.get())
            ResponseEntity.status(200).body(responseDTO)
        } else {
            ResponseEntity.notFound().build()
        }
